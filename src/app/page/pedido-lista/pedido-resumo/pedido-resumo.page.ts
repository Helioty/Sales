...
export class PedidoResumoPage implements OnInit {
  ...
  readonly produtos = new BehaviorSubject<Pagination<PedidoItens>>(null);
  ...
  separaDadosPedido(): void {
    ...
    const eve = (data: Pagination<PedidoItens> | null) => {
      this.produtos.next(data as Pagination<PedidoItens>);
  ...
  doInfinite(infinite: IonInfiniteScroll): void {
    const event = (data: Pagination<PedidoItens> | null) => {
      if (data) {
