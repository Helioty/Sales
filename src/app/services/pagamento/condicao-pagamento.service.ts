...
export class CondicaoPagamentoService {
  ...
  async setTipoPagamento(tipoPagamento: string) {
    const aResult: any = await this.pedidoService.atualizaPedido(
      'tipo_pagamento',
      tipoPagamento
    );
  ...
  async setCondicaoPagamento(opcaoSelect: OpcaoParcela, valor: any) {
    ...
    const table0 = await this.pedidoService.atualizaPedido(
      'tipo_pagamento',
      this.pedidoService.pedidoHeader.tipodoc
    );
    aResult.push(table0[0]);

    if (opcaoSelect.id !== '' && opcaoSelect.id !== undefined) {
      const table1 = await this.pedidoService.atualizaPedido(
        'condicao_pagto',
        opcaoSelect.id
      );
    ...
    if (valor !== '' && valor !== undefined) {
      const table2 = await this.pedidoService.atualizaPedido(
        'valorentrada',
        valor.toString()
      );
